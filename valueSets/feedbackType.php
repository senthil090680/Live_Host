...
$sql =("UPDATE feedback_type SET 
        feedback_type='$feedback_type'
       	WHERE id =$id");
mysql_query( $sql);
header("location:feedbackType.php?no=2&page=page");
}
}
...
$sel_query=mysql_query($sel);
        ...
        mysql_query( $sql);
        header("location:feedbackType.php?no=1&page=page");
		}
		else {
		header("location:feedbackType.php?no=18&page=page");
		}
}
  ...
  <tr height="60px;">
   ...
   <tr align="center" height="70px;">
       <td colspan="10">
        ...
        <input type="reset" name="reset" class="buttons" value="Clear" id="clear" onclick="return feedClear();"/>&nbsp;&nbsp;&nbsp;&nbsp;
...
<?php include("../include/error.php");?>
    ...
    <input type="text" name="feedback_type" value="<?php $_GET['feedback_type']; ?>" autocomplete='off' placeholder='Search By FeedbackType'/>
...
<div id="container">
	    ...
	    header("location:feedbackType.php?no=3&page=page");
		}
		?>  
        <?php
		if($_GET['submit']!='')
        ...
        $trimmed = trim($var);		
	    $qry="SELECT * FROM `feedback_type` where feedback_type like '%".$trimmed."%' order by feedback_type asc";
		}
		else
		{ 
		$qry="SELECT * FROM `feedback_type` order by feedback_type asc";  
		}
		...
		<td align="right">
        <a href="feedbackType.php?id=<?php echo $fetch['id'];?>&page=<?php echo intval($_GET['page']);?>"><img src="../images/user_edit.png" alt="" title="" width="11" height="11"/></a>&nbsp;&nbsp;&nbsp;&nbsp;
      <!--  <a href="feedbackType.php?id=<?php echo $fetch['id'];?>&delID=<?php echo $fetch['id'];?>&page=<?php echo intval($_GET['page']);?>"><img src="../images/trash.png" alt="" title="" width="11" height="11" /></a>-->
        </td>
        </tr>
		<?php $c++; $cc++; }		 
