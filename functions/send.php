...
if (!$tableName) {
...
else
  {
   ...
   if ($type == "full")  //Full mode Process
    ...
    {
     $tempQuery = "select Product_code from kd_product where KD_Code = '" . $Kdcode . "'";
     //echo $tempQuery;
	...
	$path ="Host/functions/Table/$Kdcode/$curdate";
	...
	$pathkd = "..//..//Host//functions//Table//$Kdcode//$curdate//";

    if (!is_dir($pathkd)) {
		
		$mode = 0777;
		
        mkdir($pathkd,$mode,true);
    }
	
    $date = new DateTime();
   
   writeCsv($pathkd.$tableName . "@" . $date->getTimestamp() . ".csv", $data);
...
if ($hostURL != "") {
    ...
    if ($ping > 0) {     //if Base system online
       ...
       $message= invoke($IP, "functions/load", $data,$type,$kdSpecific,$Kdcode);
        echo $message;
    }
}


function invoke($ip, $path, $data, $type,$kdSpecific,$Kdcode) {
   
    //pre($data);
    $url = "http://" . $ip . $path . ".php?remip=" . $data[0] . "&table=" . $data[1] . "&path=" . $data[2] . "&type=" . $type ."&kdSpecific=" .$kdSpecific."&Kdcode=" .$Kdcode;
   //echo $url;
	//exit;
