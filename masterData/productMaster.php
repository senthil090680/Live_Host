...
$sel_query=mysql_query($sel);
	 ...
	 $sql="INSERT IGNORE INTO `product`(`Product_code`,`Product_description1`,`Product_description_length`,`Product_display_description`,`UOM1`,`UOM2`,`UOM_Conversion`,`product_type`,`product_category`,`principal`,`Focus`,`Effective_from`,`Effective_to`,`batch_ctrl`,`brand`,`division`)
values('$Product_code','$Product_description1','$Product_description_length','$Product_display_description','$UOM1','$UOM2','$UOM_Conversion','$product_type','$product_category','$principal','$Focus','$Effective_from','$Effective_to','$batch_ctrl','$brand','$division')";
mysql_query($sql);
...
<legend><strong>Product Parameters</strong></legend>
   <table  width="100%" height="60">
    ...
    <td class="align">Brand*</td>
     <td>
      <select name="brand" id="brand"  onChange="return getprincipal();" style="width:80px">
        <option value="">--- Select ---</option>
        <?php
   ...
   <input type="hidden" name="principal" size="15" id="principal" value="<?php echo $principal ?>"  maxlength="20" autocomplete='off' readonly="readonly" />
      <input type="text"  style="width:80px" name="principalname" id="principalname" value="<?php
    ...
    <td class="align">Division*</td>
     <td>
      <select name="division" id="division" style="width:80px">
        <option value="">--- Select ---</option>
        <?php
        // Get records from database (table "name_list").
        $list=mysql_query("select * from division_master order by  division asc");

        // Show records by while loop.
        while($row_list=mysql_fetch_assoc($list)){
        ?>
        <option value="<?php echo $row_list['id']; ?>" <?php if($row_list['id']==$division){ echo "selected"; } ?>><?php echo $row_list['division']; ?></option>
        <?php
        // End while loop.
        }
        ?>
