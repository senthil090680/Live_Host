...
if(isset($_REQUEST[freqval]) && $_REQUEST[freqval] !='') {
	...
	$query_trans									=   "SELECT id,KD_Code,DSR_Code,Product_code,Transaction_Number,Sold_Quantity AS SUM_SQ,Value AS VALUE_NAIRA,Scheme_Code,Product_Scheme_Flag FROM transaction_line WHERE Transaction_Number IN ('".$transno_Total."') $prodcodecol AND Focus_Flag = 1 ORDER BY id";
	//echo $query_trans;
	//exit;
	...
	$query_trans									=   "SELECT id,KD_Code,DSR_Code,Product_code,Transaction_Number,Sold_Quantity AS SUM_SQ,Value AS VALUE_NAIRA,Scheme_Code,Product_Scheme_Flag FROM transaction_line WHERE Transaction_Number IN ('".$transno_Total."') $prodcodecol AND Focus_Flag = 0 AND Product_code = '0'  AND Scheme_Code != '' ORDER BY id";
	//echo $query_trans;
	//exit;
	$res_trans										=   mysql_query($query_trans);
	...
	while($row_trans								=   mysql_fetch_assoc($res_trans)) {
		//$transInfo[$row_trans["Transaction_Number"]]=	$row_trans;
		$transInfoRebate[$row_trans["Transaction_Number"]."-".$row_trans["id"]]	=	$row_trans;
		$transno_trans[]							=	$row_trans["Transaction_Number"];
		$kdcode_trans[]								=	$row_trans["KD_Code"];
		$dsrcode_trans[]							=	$row_trans["DSR_Code"];
		$prodcode_trans[]							=	$row_trans["Product_code"];
	...
	$array_merging		=	array_merge($transInfoRebate,$transInfo);
	...
	$array_merging	=	subval_sort_toretainkeys($array_merging,$orderbycolumns,$dir);

	//pre($array_merging);
	//exit;

	foreach($array_merging as $val_transnokey=>$val_transno) {
	...
	foreach($finalSearchInfo as $val_kd){
		...
		if($val_kd["Scheme_Code"] !='' && $val_kd["Product_code"] == '0') {
			//echo "erere<br>";
			for($y=0;$y<1000;$y++){
				//echo $actual_schemeline[$val_kd["Scheme_Code"].$val_kd["TRANS_NO"].$y]."<br>";
				if($actual_schemeline[$val_kd["Scheme_Code"].$val_kd["TRANS_NO"].$y] != ''){
					//echo ($actual_schemeline[$val_kd["Scheme_Code"].$val_kd["TRANS_NO"].$y])+($val_kd[VALUE_NAIRA])."<br>";
...
?>
<title>KD FOCUS PRODUCTS SALES PERFORMANCE</title>
...
#colors{
  ...
  <table border="1" width="100%" style="border-collapse:collapse">
	<thead>
	   <tr>
			<th align="center" colspan="10">KD FOCUS PRODUCTS SALES PERFORMANCE REPORT</th>
	  </tr>
	  <tr>
			...
			if($freq == 1) {
				echo "Daily : &nbsp;&nbsp;&nbsp;".$datevalue."&nbsp;&nbsp;&nbsp;&nbsp;KD : &nbsp;&nbsp;";
			} if($freq == 2) {
				echo "Week Start Date : &nbsp;&nbsp;&nbsp;".$datevalue."&nbsp;&nbsp;&nbsp;&nbsp;Week End Date : &nbsp;&nbsp;&nbsp;".$fifthdate."&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;KD : &nbsp;&nbsp;";
			} if($freq == 3) {
	  ...
	  <tr>
		<th align="center" style="width:30%">KD Name</th>
		<th align="center" style="width:7%">SR Name</th>
		<th align="center" style="width:7%">ASM Name</th>
		<th align="center" style="width:7%">RSM Name</th>
		<th align="center" style="width:7%">Brand</th>
		<th align="center" style="width:80%">Product</th>
		<th align="center" style="width:5%">Target
		<table  width="100%"><tr><td>Units</td><td>Naira</td></tr></table>
		</th>
		<th align="center" style="width:5%">Sales
		 <table width="100%"><tr><td>Units</td><td>Naira</td></tr></table>
		</th>
		<th align="center" style="width:5%">Difference
		 <table  width="100%"><tr><td>Units</td><td>Naira</td></tr></table>
		</th>          
  </tr>
  </thead>
...
<tr>
	 <td <?php if($reportby == 'KD_Name') { ?> style="background-color:#31859C;" <?php } ?> ><?php echo ucwords(strtolower($SearchVal[KD_Name])); ?></td>
	  ...
	  <td <?php if($reportby == 'ASM_Name') { ?> <?php } ?>><?php echo ucwords(strtolower($SearchVal[ASM_Name])); ?></td>
	  <td <?php if($reportby == 'RSM_Name') { ?><?php } ?>><?php echo ucwords(strtolower($SearchVal[RSM_Name])); ?></td>	
	  <td <?php if($reportby == 'Brand_Name') { ?><?php } ?> ><?php echo ucwords(strtolower($SearchVal[Brand_Name])); ?></td>
	  <td <?php if($reportby == 'Product_Name') { ?><?php } ?>><?php echo ucwords(strtolower($SearchVal[Product_Name])); ?></td>	
	  <td>&nbsp;
