...
if(isset($_REQUEST[kd_id]) && $_REQUEST[kd_id] !='') {
	$nextrecval		=	"WHERE (Date BETWEEN '$fromdate' AND '$todate') AND trhe.KD_Code = '$KDCodeVal' AND trhe.DSR_Code = '$dsr_id'";	
} else {
	$nextrecval		=	"";
}
$where		=	"$nextrecval";
...
<div class="statictabledev">
	<div class="con3_dev">
	 <table width="100%">
		...
		$sel_lincnt		=	"SELECT Transaction_Number FROM `transaction_line` WHERE (Transaction_Number = '$devtransactionno' AND KD_Code = '$KDCodeVal')";
		...
		$sel_feedback		=	"SELECT id FROM `feedback` WHERE Transaction_Number	= '$fetch[Transaction_Number]' AND KD_Code = '$KDCodeVal'";
		...
		?></span></td>
		<td align="center" style="width:5%"><span <?php if($devtransactiontype == 3) { ?> onclick="getlineitems('<?php echo $fetch['transaction_Reference_Number']; ?>','<?php echo $devtransactionid; ?>','<?php echo $devtransactiontype; ?>','<?php echo $fromdate; ?>','<?php echo $todate; ?>','<?php echo $KDCodeVal; ?>','<?php echo $dsr_id; ?>');" <?php } else { ?> onclick="getlineitems('<?php echo $devtransactionno; ?>','<?php echo $devtransactionid; ?>','<?php echo $devtransactiontype; ?>','<?php echo $fromdate; ?>','<?php echo $todate; ?>','<?php echo $KDCodeVal; ?>','<?php echo $dsr_id; ?>');" <?php } ?> style="cursor:pointer;cursor:hand;"><a style="color:blue;" ><?php echo $devtransactionno; ?></a></span></td>
		...
		<span><a href="javascript:void(0);" onclick="getcustomerimage('<?php echo $devtransactionno; ?>','<?php echo $KDCodeVal; ?>');" style="cursor:pointer;cursor:hand;">Image Available</a></span>  
		...
		<span><a href="javascript:void(0);" onclick="getcustomersig('<?php echo $devtransactionno; ?>','<?php echo $KDCodeVal; ?>');" style="cursor:pointer;cursor:hand;">Signature Available</a></span>  
